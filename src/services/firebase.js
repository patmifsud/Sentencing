...
function startGameFieldsSnapshot(
  setCurrentPhase, 
  setWonSentence, 
  gameId){
...
function startStorySnapshot(setStory, gameId){
    ...
    db.collection("games").doc(gameId).collection("story")
      .onSnapshot(function (querySnapshot) {
        let allStories = [];
        if (querySnapshot.docs.length > 0) {
          querySnapshot.docs.forEach((doc) => {
...
function startSentencesSnapshot(setTempSentences, gameId){
    ...
    db.collection("games").doc(gameId).collection("sentences")
    .onSnapshot(function (querySnapshot) {
      let allSentences = [];
      if (querySnapshot.docs.length > 0) {
        querySnapshot.docs.forEach((doc) => {
...
function startFBSnapshots(
  ...
  setTempSentences,
  gameId){
    startGameFieldsSnapshot(
      setCurrentPhase, setWonSentence, gameId);
   startPlayersSnapshot(
      setPlayerId, gameId);
   startStorySnapshot(
      setStory, gameId);
   startSentencesSnapshot(
      setTempSentences, gameId);
}
